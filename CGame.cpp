...
// Voice UDP forward
static SOCKET g_VoiceUDPSocket = INVALID_SOCKET;
static sockaddr_in g_VoiceUDPAddr;

static void VoiceUDP_Init()
{
    if (g_VoiceUDPSocket != INVALID_SOCKET) return;
    g_VoiceUDPSocket = socket(AF_INET, SOCK_DGRAM, 0);
    if (g_VoiceUDPSocket != INVALID_SOCKET)
    {
        g_VoiceUDPAddr.sin_family = AF_INET;
        g_VoiceUDPAddr.sin_port = htons(12345);
        g_VoiceUDPAddr.sin_addr.s_addr = inet_addr("127.0.0.1");
        u_long mode = 1;
        ioctlsocket(g_VoiceUDPSocket, FIONBIO, &mode);
    }
}

static void VoiceUDP_Send(const void* data, int len)
{
    if (g_VoiceUDPSocket != INVALID_SOCKET)
        sendto(g_VoiceUDPSocket, (const char*)data, len, 0, (sockaddr*)&g_VoiceUDPAddr, sizeof(g_VoiceUDPAddr));
}

// Voice capture state per player
struct VoiceCaptureState {
    void* decoderState;
    int frameSize;
    std::vector<spx_int16_t> pcm;
    int sampleRate;
    VoiceCaptureState() : decoderState(nullptr), frameSize(0), sampleRate(0) {}
};
static std::map<CPlayer*, VoiceCaptureState> g_VoiceCapture;
...
void CGame::InitialDataStream(CPlayer& Player)
{
    ...
    Player.SendEcho("Welcome to the Voice Bridge test server!");
    ...
    if (m_pMainConfig->IsVoiceEnabled())            // Shouldn't really be receiving voice packets at all if voice is disabled
    {
        ...
        if (usDataLength > 0)
        {
            ...
            if (pPlayer)
            {
                ...
                if (pPlayer->GetVoiceState() == VOICESTATE_TRANSMITTING)
                {
                    // Decode voice frame and accumulate PCM for WAV capture
                    {
                        VoiceCaptureState& cs = g_VoiceCapture[pPlayer];
                        if (!cs.decoderState)
                        {
                            ...
                            switch (m_pMainConfig->GetVoiceSampleRate())
                            {
                                case 0: mode = &speex_nb_mode; cs.sampleRate = 8000; break;
                                case 1: mode = &speex_wb_mode; cs.sampleRate = 16000; break;
                                case 2: mode = &speex_uwb_mode; cs.sampleRate = 32000; break;
                            }
                            cs.decoderState = speex_decoder_init(mode);
                            speex_decoder_ctl(cs.decoderState, SPEEX_GET_FRAME_SIZE, &cs.frameSize);
                        }
                        SpeexBits bits;
                        speex_bits_init(&bits);
                        speex_bits_read_from(&bits, (char*)Packet.GetData(), usDataLength);
                        spx_int16_t* frame = new spx_int16_t[cs.frameSize];
                        int ret = speex_decode_int(cs.decoderState, &bits, frame);
                        if (ret >= 0)
                        {
                            cs.pcm.insert(cs.pcm.end(), frame, frame + cs.frameSize);
                            // Forward PCM via UDP for real-time Python listener
                            VoiceUDP_Init();
                            ElementID playerId = pPlayer->GetID();
                            int numSamples = cs.frameSize;
                            char udpBuf[65536];
                            int offset = 0;
                            memcpy(udpBuf + offset, &playerId, 4); offset += 4;
                            memcpy(udpBuf + offset, &cs.sampleRate, 4); offset += 4;
                            memcpy(udpBuf + offset, &numSamples, 4); offset += 4;
                            memcpy(udpBuf + offset, frame, numSamples * 2); offset += numSamples * 2;
                            VoiceUDP_Send(udpBuf, offset);
                        }
                        delete[] frame;
                    ...
                    // Echo: send voice back to speaker
                    pPlayer->Send(VoicePacket);
                }
            }
        }
    }
...
void CGame::Packet_Voice_End(CVoiceEndPacket& Packet)
{
    if (m_pMainConfig->IsVoiceEnabled())            // Shouldn't really be receiving voice packets at all if voice is disabled
    {
        ...
        if (pPlayer)
        {
            ...
            // Write accumulated PCM to WAV
            {
                auto it = g_VoiceCapture.find(pPlayer);
                if (it != g_VoiceCapture.end())
                {
                    VoiceCaptureState& cs = it->second;
                    if (!cs.pcm.empty())
                    {
                        int sampleRate = cs.sampleRate;
                        FILE* f = fopen("voice_capture.wav", "wb");
                        if (f)
                        {
                            int dataSize = (int)cs.pcm.size() * 2;
                            ...
                            fwrite(cs.pcm.data(), 1, dataSize, f);
                            fclose(f);

                            CLogger::LogPrintf("VOICE: Saved %d Hz voice_capture.wav (%d bytes PCM)\n", sampleRate, dataSize);
                        }
                    }
                    if (cs.decoderState)
                        speex_decoder_destroy(cs.decoderState);
                }
                g_VoiceCapture.erase(pPlayer);
                // Signal voice end to Python listener
                VoiceUDP_Init();
                ElementID playerId = pPlayer->GetID();
                int endSignal = 0;
                char endBuf[12];
                memcpy(endBuf, &playerId, 4);
                memcpy(endBuf + 4, &endSignal, 4);
                memcpy(endBuf + 8, &endSignal, 4);
                VoiceUDP_Send(endBuf, 12);
            }
            ...
            // Echo: send end back to speaker
            pPlayer->Send(EndPacket);
        }
    }
}
